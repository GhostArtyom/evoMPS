...
def herm_sqrt_inv(x, zero_tol=1E-15, sanity_checks=False, return_rank=False, sc_data=''):
    if isinstance(x,  mm.eyemat):
        ...
        if sanity_checks:
            if ev.min() < -zero_tol:
                print("warning: Sanity Fail in herm_sqrt_inv(): Throwing away negative eigenvalues! %s %s" %
                            (ev.min(), sc_data))
            
            if not np.allclose(x_sqrt.dot(x_sqrt), x):
                print("warning: Sanity Fail in herm_sqrt_inv(): x_sqrt is bad! %s %s" %
            ...
            if not np.allclose(x_sqrt.dot(x_sqrt_i), eye - nulls):
                print("warning: Sanity Fail in herm_sqrt_inv(): x_sqrt_i is bad! %s %s" %
...
def calc_Vsh(A, r_s, sanity_checks=False):
    ...
    if sanity_checks:
        if not sp.allclose(mm.mmul(Vconj.conj(), R), 0):
            print("warning: Sanity Fail in calc_Vsh!: VR != 0")
        if not sp.allclose(mm.mmul(Vconj, Vconj.conj().T), sp.eye(Vconj.shape[0])):
            print("warning: Sanity Fail in calc_Vsh!: V H(V) != eye")
    ...
    if sanity_checks:
        ...
        if not sp.allclose(M, 0):
            print("warning: Sanity Fail in calc_Vsh!: Bad Vsh")
...
def calc_Vsh_l(A, lm1_sqrt, sanity_checks=False):    
    ...
    if sanity_checks:
        if not sp.allclose(L.dot(V), 0):
            print("warning: Sanity Fail in calc_Vsh_l!: LV != 0")
        if not sp.allclose(V.conj().T.dot(V), sp.eye(V.shape[1])):
            print("warning: Sanity Fail in calc_Vsh_l!: V H(V) != eye")
    ...
    if sanity_checks:
        ...
        if not sp.allclose(M, 0):
            print("warning: Sanity Fail in calc_Vsh_l!: Bad Vsh")
...
def herm_fac_with_inv(A, lower=False, zero_tol=1E-15, return_rank=False, 
    ...
    if force_evd:
        ...
        if sanity_checks:
            ...
            if ev.min() < -zero_tol:
                print("warning: Sanity fail in herm_fac_with_inv(): Discarding negative eigenvalues! %s %s" %
                            (ev.min(), sc_data))
    ...
    if sanity_checks:
        if not sp.allclose(A, A.conj().T, atol=1E-13, rtol=1E-13):
            print("warning: Sanity fail in herm_fac_with_inv(): A is not Hermitian! %s %s" %
    ...
    for n in range(len(A) - 1, 0, -1):
        ...
        if sanity_checks:
            ...
            if not sp.allclose(r_nm1_, r[n - 1].A, atol=1E-13, rtol=1E-13):
                print("warning: Sanity Fail in restore_RCF_r!: r is bad! %s %s" %
...
def restore_RCF_r(A, r, G_n_i, zero_tol=1E-15, sanity_checks=False, sc_data=''):
    ...
    if sanity_checks:     
        ...
        if not sp.allclose(GiG.dot(As).dot(G_n_i), 
            ...
            print("warning: Sanity Fail in restore_RCF_r!: Bad GT! %s %s" %
    ...
    if sanity_checks:
        ...
        if not sp.allclose(r_nm1_, r_nm1.A, atol=1E-13, rtol=1E-13):
            print("warning: Sanity Fail in restore_RCF_r!: r != g old_r gH! %s %s" %
...
def restore_RCF_l(A, lm1, Gm1, sanity_checks=False):
    ...
    if sanity_checks:
        ...
        if not sp.allclose(l_, l, atol=1E-12, rtol=1E-12):
            print("warning: Sanity Fail in restore_RCF_l!: l is bad!")
            print(la.norm(l_ - l))
    ...
    for n in range(1, len(A)):
        ...
        if sanity_checks:
            ...
            if not sp.allclose(l_, l[n].A, atol=1E-13, rtol=1E-13):
                print("warning: Sanity Fail in restore_LCF_l_seq!: l is bad")
                print(la.norm(l_ - l[n].A))
...
def restore_LCF_l(A, lm1, Gm1, sanity_checks=False, zero_tol=1E-15):
    ...
    if sanity_checks:
        ...
        if not sp.allclose(G.dot(Gi), eye, atol=1E-13, rtol=1E-13):
            print("warning: Sanity Fail in restore_LCF_l!: Bad GT!")
    ...
    if sanity_checks:
        ...
        if not sp.allclose(l_, l.A, atol=1E-13, rtol=1E-13):
            print("warning: Sanity Fail in restore_LCF_l!: l is bad")
            print(la.norm(l_ - l))
...
def restore_LCF_r(A, r, Gi, sanity_checks=False):
    ...
    if sanity_checks:
        ...
        if not sp.allclose(rm1_, rm1, atol=1E-12, rtol=1E-12):
            print("warning: Sanity Fail in restore_LCF_r!: r is bad!")
